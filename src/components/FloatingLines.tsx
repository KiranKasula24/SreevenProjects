...
const vec3 GOLD  = vec3(251.0, 191.0, 36.0) / 255.0;
const vec3 BLUE  = vec3(59.0, 130.0, 246.0) / 255.0;
const vec3 CYAN  = vec3(6.0, 182.0, 212.0) / 255.0;
...
vec3 background_color(vec2 uv) {
  ...
  col += mix(CYAN, BLACK, smoothstep(0.0, 1.0, abs(m + 0.6)));
  return col * 0.45;
}
...
export default function FloatingLines({
  linesGradient = ["#3b82f6", "#06b6d4", "#fbbf24", "#1d4ed8"],
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      topLineCount: { value: Math.min(topLineCount, 32) },
      middleLineCount: { value: Math.min(middleLineCount, 32) },
      bottomLineCount: { value: Math.min(bottomLineCount, 32) },
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      targetInfluenceRef.current = interactive ? 1.0 : 0.0;
